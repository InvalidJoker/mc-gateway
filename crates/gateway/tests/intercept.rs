//! Port interception, driven without TPROXY: connections are accepted on a
//! plain listener and handed to the intercept handler with the fake server's
//! address as the "original destination", exactly what TPROXY would report.
//!
//! The kernel side — the rules, fail-open, Docker's DNAT — is covered by
//! test-infra/intercept-check/.

mod common;

use std::{net::SocketAddr, path::PathBuf, sync::Arc, time::Duration};

use common::{Behaviour, FakeBackend, handshake, login_start, paper_status, read_packet};
use mc_config::Config;
use mc_gateway::{app::App, intercept};
use mc_protocol::{NextState, Reader, encode_packet, status::STATUS_REQUEST_ID};
use tokio::{
...
async fn node(config: &str, server: SocketAddr) -> SocketAddr {
    let loaded = Config::parse(config, "test").expect("config parses");
    let app = App::start(loaded, PathBuf::from("test.yaml"));
    let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
    let address = listener.local_addr().unwrap();
    tokio::spawn(async move {
        loop {
            let (stream, peer) = listener.accept().await.unwrap();
            tokio::spawn(intercept::handle(stream, peer, server, Arc::clone(&app)));
        }
    });
    address
}
...
async fn status_ping(address: SocketAddr) -> serde_json::Value {
    ...
    stream.write_all(&handshake(767, "node.example.net", 30123, NextState::Status)).await.unwrap();
...
async fn the_ad_replaces_line_two_and_nothing_else() {
    let customer =
        FakeBackend::start(Behaviour::Status(paper_status("Steve's SMP\\nwhitelist on", 4))).await;
    let address = node(WITH_AD, customer.address).await;
...
async fn without_an_ad_the_response_is_byte_identical() {
    ...
    let customer = FakeBackend::start(Behaviour::Status(json.clone())).await;
    let address = node(WITHOUT_AD, customer.address).await;
...
async fn a_login_reaches_the_server_verbatim() {
    let customer = FakeBackend::start(Behaviour::Echo).await;
    let address = node(WITH_AD, customer.address).await;

    let mut sent = handshake(767, "node.example.net", 30123, NextState::Login);
    sent.extend_from_slice(&login_start("Steve"));
    let mut stream = TcpStream::connect(address).await.unwrap();
    stream.write_all(&sent).await.unwrap();

    let seen = customer.wait_for_payload_connections(1).await;
    assert_eq!(seen[0].head, sent, "not a byte added, removed or changed");
...
async fn a_server_that_speaks_first_is_not_kept_waiting() {
    ...
    let customer = FakeBackend::start(Behaviour::Banner(b"SSH-2.0-OpenSSH_9.9\r\n".to_vec())).await;
    let address = node(WITH_AD, customer.address).await;
...
async fn a_pre_1_7_ping_passes_through() {
    ...
    let seen = customer.wait_for_payload_connections(1).await;
    assert_eq!(seen[0].head, vec![0xFE, 0x01]);
}
...
async fn a_stopped_server_looks_like_a_closed_port() {
    ...
    let address = node(WITH_AD, stopped).await;
...
async fn a_modpack_sized_status_response_still_gets_the_ad() {
    ...
    let customer = FakeBackend::start(Behaviour::Status(json)).await;
    let address = node(WITH_AD, customer.address).await;
...
async fn an_unparseable_status_response_is_forwarded_as_is() {
    let customer = FakeBackend::start(Behaviour::Status("definitely not json".into())).await;
    let address = node(WITH_AD, customer.address).await;
...
async fn the_latency_ping_after_the_ad_still_works() {
    let customer = FakeBackend::start(Behaviour::Status(paper_status("hi", 0))).await;
    let address = node(WITH_AD, customer.address).await;
    ...
    let mut ping = mc_protocol::Writer::new();
    ping.i64(987_654_321);
    stream.write_all(&encode_packet(0x01, ping.as_slice())).await.unwrap();
