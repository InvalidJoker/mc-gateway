//! Port interception on a hosting node.
//!
...
use mc_protocol::{
    ...
    legacy,
    status::{STATUS_RESPONSE_ID, encode_status_response},
    varint,
};
use mc_config::Motd;
use tokio::{
...
use crate::{app::App, motd, observe, pipe};
            ...
            accepted = listener.accept() => match accepted {
                Ok((stream, peer)) => {
                    ...
                    tokio::spawn(async move {
                        handle(stream, peer, original, app).await;
                        drop(token);
                    });
                }
                Err(err) => {
...
pub async fn handle(client: TcpStream, peer: SocketAddr, original: SocketAddr, app: Arc<App>) {
    observe::connection_opened();
    let outcome = serve(client, peer, original, &app).await;
...
async fn serve(
    ...
    let _ = mc_forwarding::enable_keepalive(&client);

    let mut upstream =
        match mc_forwarding::connect(original, Origin::Intercepted(peer), config.timeouts.connect)
            .await
        {
            Ok(upstream) => upstream,
            Err(err) => return Outcome::Unreachable(err.kind()),
        };
    let _ = mc_forwarding::enable_keepalive(&upstream);
    ...
    let bytes = pipe::run(client, upstream, None).await.ok();
    Outcome::Piped { kind, rewritten, bytes }
}
...
fn classify(buf: &[u8]) -> Option<Sniff> {
    ...
    let mut pos = 0;
    match varint::read_varint(buf, &mut pos) {
...
mod tests {
    use super::*;
    use mc_protocol::{Writer, encode_packet};
    ...
    fn a_huge_declared_length_is_not_a_handshake() {
        let mut buf = Vec::new();
        varint::write_varint(&mut buf, 5000);
